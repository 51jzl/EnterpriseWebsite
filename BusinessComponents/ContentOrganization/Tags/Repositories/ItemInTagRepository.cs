    ...
    public class ItemInTagRepository : Repository<ItemInTag>, IItemInTagRepository
    {
        ...
        public int AddTagsToItem(string[] tagNames, string tenantTypeId, long ownerId, long itemId)
        {
            ...
            foreach (string tagName in tagNames)
            {
                ...
                var tag = dao.FirstOrDefault<Tag>(sql);
                if (tag == null)
                    sqls.Add(Sql.Builder.Append("insert into tn_Tags (TenantTypeId,TagName,DateCreated,Description,FeaturedImage,IsFeatured,ItemCount,OwnerCount,AuditStatus) values (@0,@1,@2,@3,@4,@5,@6,@7,@8)", tenantTypeId, tagName, DateTime.UtcNow, string.Empty, string.Empty, 0, 0, 0, AuditStatus.Success));
                //创建标签与用户的关联
                sql = Sql.Builder;
                sql.From("tn_TagsInOwners")
                   .Where("TenantTypeId = @0", tenantTypeId)
                ...
                if (tagInOwner == null)
                {
                    ...
                    dao.Execute(Sql.Builder.Append("insert into tn_TagsInOwners (TenantTypeId,TagName,OwnerId,ItemCount) values (@0,@1,@2,1)", tenantTypeId, tagName, ownerId));
                    tagInOwner = dao.FirstOrDefault<TagInOwner>(sql);
                }
                else
                {
        ...
        public int DeleteTagFromItem(long itemId, long tagInOwnerId)
        {
            ...
            ItemInTag tagInfo = dao.FirstOrDefault<ItemInTag>(sql);
            int affectCount = 0;
        ...
        public IEnumerable<long> GetItemIds(string tagName, string tenantTypeId, long? ownerId)
        {
            ...
            itemIds = cacheService.Get<List<long>>(cacheKey.ToString());
            if (itemIds == null)
            {
        ...
        public IEnumerable<long> GetTagIdsOfItem(long itemId, string tenantTypeId)
        {
            ...
            List<long> tagIds = cacheService.Get<List<long>>(cacheKey.ToString());
        ...
        public IEnumerable<long> GetItemInTagIdsOfItem(long itemId, string tenantTypeId)
        {
            ...
            List<long> itemInTagIds = cacheService.Get<List<long>>(cacheKey.ToString());
        ...
        public IEnumerable<long> GetTagInOwnerIdsOfItem(long itemId, long ownerId, string tenantTypeId)
        {
            ...
            List<long> tagInOwnerIds = cacheService.Get<List<long>>(cacheKey.ToString());
            ...
            if (tagInOwnerIds == null)
            {
                ...
                tagInOwnerIds = CreateDAO().FetchFirstColumn(sql).Cast<long>().ToList();
