    ...
    public class TagRepository<T> : Repository<T>, ITagRepository<T> where T : Tag
    {
        ...
        public IEnumerable<string> GetTopTagNames(string tenantTypeId, long ownerId, string keyword, int topNumber)
        {
            ...
            sql.Select("tn_Tags.TagName")
               .From("tn_Tags")
               .InnerJoin("tn_TagsInOwners")
               .On("tn_Tags.TagName = tn_TagsInOwners.TagName and tn_Tags.TenantTypeId = tn_TagsInOwners.TenantTypeId")
               .Where("tn_Tags.TenantTypeId = @0", tenantTypeId)
               .Where("tn_TagsInOwners.OwnerId = @0", ownerId)
               .Where("tn_Tags.TagName like @0", "%" + StringUtility.StripSQLInjection(keyword) + "%")
               .OrderBy("tn_TagsInOwners.OwnerId Desc")
               .OrderBy("tn_TagsInOwners.ItemCount Desc")
               .OrderBy("tn_Tags.OwnerCount")
