...
namespace Victornet.Common.Repositories
{
    ...
    public class TagInOwnerRepository : Repository<TagInOwner>, ITagInOwnerRepository
    {
        ...
        public override int Delete(TagInOwner entity)
        {
            ...
            sqls.Add(Sql.Builder.Append("update tn_Tags Set OwnerCount = OwnerCount - 1 where TenantTypeId = @0 and TagName = @1", entity.TenantTypeId, entity.TagName));
        ...
        public int AddTagInOwner(string tagName, string tenantTypeId, long ownerId)
        {
            ...
            if (!string.IsNullOrEmpty(tagName) && !string.IsNullOrEmpty(tenantTypeId) && ownerId >= 0)
            {
                ...
                if (affectCount == 0)
                {
                    ...
                    sqls.Add(Sql.Builder.Append("INSERT INTO tn_TagsInOwners (OwnerId, TagName, TenantTypeId) VALUES (@0,@1,@2)", ownerId, tagName, tenantTypeId));
        ...
        public long AddTagInOwner(TagInOwner tagInOwner)
        {
            ...
            if (id > 0)
            {
                dao.Execute(Sql.Builder.Append("update tn_Tags Set OwnerCount = OwnerCount + 1 where TenantTypeId = @0 and TagName = @1", tagInOwner.TenantTypeId, tagInOwner.TagName));
