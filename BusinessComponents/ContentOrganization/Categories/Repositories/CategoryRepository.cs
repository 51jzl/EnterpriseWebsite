...
namespace Victornet.Common.Repositories
{
    ...
    public class CategoryRepository<T> : Repository<T>, ICategoryRepository<T> where T : Category
    {
        ...
        public IEnumerable<T> GetOwnerRootCategories(long ownerId, string tenantTypeId)
        {
           return  GetTopEntities(SecondaryMaxRecords, Caching.CachingExpirationType.ObjectCollection,
            () =>
            {
                //获取缓存
        ...
        public IEnumerable<T> GetOwnerCategories(long ownerId, string tenantTypeId)
        {
            ...
            GetTopEntities(SecondaryMaxRecords, Caching.CachingExpirationType.ObjectCollection,
            () =>
            {
                //获取缓存
                return cacheKey.ToString();
        ...
        public PagingDataSet<T> GetCategories(PubliclyAuditStatus? publiclyAuditStatus, string tenantTypeId, string keyword, int pageIndex)
        {
            ...
            {
                sql.Where("CategoryName like @0", "%" + StringUtility.StripSQLInjection(keyword) + "%");
            }
