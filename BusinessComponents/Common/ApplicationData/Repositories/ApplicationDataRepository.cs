...
namespace Victornet.Common.Repositories
{
    ...
    public class ApplicationDataRepository : Repository<ApplicationData>, IApplicationDataRepository
    {
        ...
        public IEnumerable<ApplicationData> GetAll(int applicationId, string tenantTypeId = "")
        {
            ...
            List<int> applicationDataIds = cacheService.Get<List<int>>(cacheKey);
            if (applicationDataIds == null)
            {
                ...
                applicationDataIds = CreateDAO().Fetch<int>(sql);
                cacheService.Add(cacheKey, applicationDataIds, CachingExpirationType.RelativelyStable);
            }
            return PopulateEntitiesByEntityIds<int>(applicationDataIds);
        }
