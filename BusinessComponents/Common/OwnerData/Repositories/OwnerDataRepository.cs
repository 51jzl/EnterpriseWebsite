...
namespace Victornet.Common.Repositories
{
    ...
    public class OwnerDataRepository : Repository<OwnerData>, IOwnerDataRepository
    {
        ...
        public IEnumerable<long> GetTopOwnerIds(string dataKey, string tenantTypeId, int topNumber, OwnerData_SortBy? sortBy = null)
        {
            ...
            if (peic == null)
            {
                IEnumerable<object> ownerIds = CreateDAO().FetchTop<long>(SecondaryMaxRecords, sql).Cast<object>();
        ...
        /// <returns></returns>
        public Dictionary<string, long> Gets(long ownerId, string tenantTypeId, IEnumerable<string> dataKeys)
        {
            string cacheKey = RealTimeCacheHelper.GetListCacheKeyPrefix(CacheVersionType.AreaVersion, "OwnerId", ownerId) + string.Join("-", dataKeys);
            Dictionary<string, long> dictionary = cacheService.Get<Dictionary<string, long>>(cacheKey);
            if (dictionary == null)
            {
                Sql sql = Sql.Builder;
                sql.Select("*")
                   ...
                   .Where("OwnerId = @0", ownerId)
                   .Where("TenantTypeId = @0", tenantTypeId)
                   .Where("DataKey in (@dataKeys)", new { dataKeys = dataKeys });
                var ownerDatas = CreateDAO().Fetch<OwnerData>(sql);
                dictionary = new Dictionary<string, long>();
                foreach (var ownerData in ownerDatas)
                {
                    dictionary[ownerData.Datakey] = ownerData.LongValue;
                }
                cacheService.Add(cacheKey, dictionary, CachingExpirationType.UsualObjectCollection);
            }
            return dictionary;
        }
