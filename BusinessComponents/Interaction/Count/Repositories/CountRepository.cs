...
namespace Victornet.Common
{
    ...
    public class CountRepository : ICountRepository
    {
        ...
        public void CheckCountTable(string tenantTypeId)
        {
            ...
            {
                builder.AppendFormat(@"CREATE TABLE IF NOT EXISTS `{0}`(
                                        CountId BigInt NOT NULL AUTO_INCREMENT,
                                        OwnerId BigInt NOT NULL,
                                        ObjectId BigInt NOT NULL,
                                        CountType Varchar(64) NOT NULL,
                                        StatisticsCount Int(11) NOT NULL,
                                        KEY `IX_CountType` (`CountType`),
                                        KEY `IX_ObjectId` (`ObjectId`),
                                        KEY `IX_OwnerId` (`OwnerId`),
                                        KEY `IX_StatisticsCount` (`StatisticsCount`),
                                        PRIMARY KEY (`CountId`)
                                        )ENGINE=innodb,DEFAULT CHARSET=utf8;", GetTableName_Counts(tenantTypeId));
            }
        ...
        public void CheckCountPerDayTable(string tenantTypeId)
        {
            ...
            if (dbtype.StartsWith("MySql"))
            {
                builder.AppendFormat(@"CREATE TABLE IF NOT EXISTS `{0}`(
                                        ...
                                        ReferenceDay Int(11) NOT NULL,
                                        StatisticsCount Int(11) NOT NULL,
                                        CountType Varchar(64) NOT NULL,
                                        KEY `IX_ObjectId` (`ObjectId`),
                                        KEY `IX_OwnerId` (`OwnerId`),
                                        KEY `IX_ReferenceDay` (`ReferenceDay`),
                                        KEY `IX_ReferenceMonth` (`ReferenceMonth`),
                                        KEY `IX_ReferenceYear` (`ReferenceYear`),
                                        PRIMARY KEY (`Id`)
                                        )ENGINE=innodb,DEFAULT CHARSET=utf8;", GetTableName_CountsPerDay(tenantTypeId));
            }
            //else if (dbtype.StartsWith("SqlCe"))
            //{
            //}
  ...
  ", GetTableName_CountsPerDay(tenantTypeId));
        ...
        public void DeleteTrashCount()
        {
            ...
            foreach (var tenantType in tenantTypes)
            {
                ...
                .Where("not exists (select 1 from " + pd.TableName + " where ObjectId = " + pd.PrimaryKey + ")"));
            }
        ...
        public Dictionary<long, int> Gets(string tenantTypeId, string countType, IEnumerable<long> objectIds)
        {
            ...
            foreach (var objectId in objectIds)
            {
                ...
                int? count = cacheService.GetFromFirstLevel(cacheKey) as int?;
                if (count != null)
                    dictionary[objectId] = count.Value;
                else
                    list.Add(objectId);
            }
            if (list.Count > 0)
