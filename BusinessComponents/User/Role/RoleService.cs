...
namespace Victornet.Common
{
    ...
    public class RoleService
    {
        ...
        public IEnumerable<Role> GetRoles(IEnumerable<string> roleNames)
        {
            return roleRepository.PopulateEntitiesByEntityIds(roleNames);
        }
        ...
        public void AddUserToRoles(long userId, List<string> roleNames)
        {
            ...
            IEnumerable<string> oldRoleNames = GetRoleNamesOfUser(userId);
            bool nameIsChange = false;
            //done:zhangp,by victor:以下代码有问题，当oldRoleNames.Count()为0时，不会为用户添加角色
        ...
        {
            IEnumerable<string> userRoleNames = GetRoleNamesOfUser(userId);
            return userRoleNames.Any(r => roleNames.Contains(r));
        }
